...
#include <QObject>
#include <QLabel>
#include <QToolButton>  //class QToolButton;
class PlotSettings;

class Plotter : public QWidget//定义Plotter类，用来创建绘图区
{
    Q_OBJECT

public:
    Plotter(QWidget *parent=0);//构造
    ~Plotter();//析构
    void setPlotSettings(const PlotSettings &settings);//设置绘图区域
    void setCurveData(const QVector<QPointF> &data);//设置曲线点坐标数据  //这里的QPointF是一个具有浮点数形式的QPoint

    QSize minimumSizeHint() const;
    QSize sizeHint() const;


    void setCorrds(double x,double y,int z,int hx);//设置Y轴最大、最小值和X轴点数,z是横坐标点数

    void SetBeginTime(QString data);//设置起始刻度
